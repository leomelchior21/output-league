...
export default function LearningCard({ levelId = 1, onContinue, code, output }: { levelId?: number; onContinue: () => void; code: string[]; output: string[] }) {
  const recap = recaps[levelId as keyof typeof recaps] ?? recaps[1];
  return <Modal label={recap.label} className="learning-modal"><span className="eyebrow success-text">KNOWLEDGE UNLOCKED · PYTHON {String(levelId).padStart(2, '0')}</span><h2>{recap.title}</h2><p>{recap.intro}</p><CodeDemo code={code} output={output} /><div className="lesson-takeaways">{recap.points.map(([title, detail]) => <p key={title}><b>{title}</b> {detail}</p>)}</div><button className="primary-button" onClick={onContinue}>SEE MY SCORE <ArrowRight size={20} /></button></Modal>;
}
