...
export default function Game({ levelId, onJourney, settings, onSettingsChange, entranceReady }: { levelId: number; onJourney: () => void; settings: SettingsValue; onSettingsChange: (s: SettingsValue) => void; entranceReady: boolean }) {
  const level = pythonLevels[levelId - 1];
  const host = useRef<HTMLDivElement>(null);
  const controls = useRef<ControlState>({ x: 0, y: 0, boost: false, kick: false, kickRequested: false, nextRoundRequested: false, paused: true, reducedMotion: settings.reducedMotion, settings });
  const audio = useRef(new GameAudio());
  const [snapshot, setSnapshot] = useState<MatchSnapshot>(() => new Match(makeLevelRounds(levelId)).snapshot());
  ...
  useEffect(() => {
    ...
    void Promise.all([import('phaser'), import('../game/ArenaScene')]).then(([{ default: Phaser }, { ArenaScene }]) => {
      if (disposed) return;
      game = new Phaser.Game({ type: Phaser.CANVAS, fps: { target: 60, smoothStep: false }, parent: host.current!, width: 1200, height: 800, backgroundColor: '#081d32', antialias: true, scale: { mode: Phaser.Scale.FIT, autoCenter: Phaser.Scale.CENTER_BOTH }, input: { activePointers: 3 }, scene: new ArenaScene({ levelId, controls: controls.current, audio: sound, onSnapshot: setSnapshot, onFeedback: setFeedback, onReady: () => setReady(true), onIntroComplete: () => setIntro(false), onKickoffChange: setKickoff, onAwaitingNext: setAwaitingNext }) });
  ...
  useEffect(() => {
    ...
    saveProgress(snapshot.score, snapshot.stars, levelId);
    const timer = setTimeout(() => setRecap(true), 2400); return () => clearTimeout(timer);
  }, [snapshot.levelComplete, snapshot.score, snapshot.stars, levelId]);
  useEffect(() => {
  ...
  const replay = () => { controls.current.paused = true; controls.current.nextRoundRequested = false; setRecap(false); setResults(false); setPaused(false); setFeedback(null); setKickoff(true); setAwaitingNext(false); setSnapshot(new Match(makeLevelRounds(levelId)).snapshot()); setRun(r => r + 1); };
  const nextRound = () => { controls.current.nextRoundRequested = true; setAwaitingNext(false); setFeedback(null); };
  return <main className={`game-screen screen-enter ${kickoff ? 'kickoff-active' : ''} ${intro ? 'arena-intro-active' : ''}`}>
    <div className="phaser-host" ref={host} role="img" aria-label="Large top-down octagonal arena with six goals and a camera following your car. Edge labels locate the ball and outputs. Steer and kick the ball into the matching output." />
    {ready && intro && !tutorial && <div className="arena-intro"><span className="eyebrow">PYTHON · LEVEL {String(levelId).padStart(2, '0')}</span><h2>{level.title}.<br /><span>ONE RIGHT OUTPUT.</span></h2><p>Read the code. Find your line.</p><button className="secondary-button" onClick={() => { controls.current.skipIntro = true; }}>KICK OFF <Play size={16} /></button></div>}
    ...
    <header className="game-hud"><div className={`code-panel ${snapshot.challenge.outputs.length > 1 ? 'mastery-code' : ''} ${snapshot.challenge.code.length > 2 ? 'expanded-code' : ''}`}><div className="code-panel-label"><span><Terminal size={14} /> PYTHON · LEVEL {String(levelId).padStart(2, '0')}</span><span>{snapshot.challenge.outputs.length > 1 ? `OUTPUT ${snapshot.phase + 1} / ${snapshot.challenge.outputs.length}` : 'WHAT GETS PRINTED?'}</span></div>{snapshot.challenge.code.map((line, i) => { const printOrder = snapshot.challenge.code.slice(0, i + 1).filter(codeLine => codeLine.startsWith('print(')).length - 1; const done = line.startsWith('print(') && printOrder < snapshot.phase; return <div className={`code-line ${done ? 'line-done' : ''}`} key={`${i}-${line}`}><span className="line-number">{done ? <Check size={17} /> : i + 1}</span><code>{line.startsWith('print(') ? <><span className="function">print</span>(<span className={line.includes('"') ? 'string' : 'number'}>{line.slice(6, -1)}</span>)</> : line}</code>{done && <Check size={18} className="success-text" />}</div>; })}</div>
    ...
    {recap && <LearningCard levelId={levelId} code={snapshot.challenge.code} output={snapshot.challenge.outputs} onContinue={() => { setRecap(false); setResults(true); }} />}
    {results && <Results levelId={levelId} state={snapshot} replay={replay} journey={onJourney} />}
  </main>;
}
