...
export default function Game({ language, levelId, onJourney, settings, onSettingsChange, entranceReady }: { language: Language; levelId: number; onJourney: () => void; settings: SettingsValue; onSettingsChange: (s: SettingsValue) => void; entranceReady: boolean }) {
  ...
  const controls = useRef<ControlState>({ x: 0, y: 0, boost: false, kick: false, kickRequested: false, pointer: { x: 0, y: 0, active: false }, nextRoundRequested: false, paused: true, reducedMotion: settings.reducedMotion, settings });
  const audio = useRef(new GameAudio());
  const [snapshot, setSnapshot] = useState<MatchSnapshot>(() => new Match(makeLevelRounds(levelId, Math.random, language)).snapshot());
  ...
  const [kickoff, setKickoff] = useState(true); const [awaitingNext, setAwaitingNext] = useState(false);
  ...
  useEffect(() => {
    ...
    void Promise.all([import('phaser'), import('../game/ArenaScene')]).then(([{ default: Phaser }, { ArenaScene }]) => {
      if (disposed) return;
      game = new Phaser.Game({ type: Phaser.CANVAS, fps: { target: 60, smoothStep: false }, parent: host.current!, width: 1200, height: 800, backgroundColor: '#081d32', antialias: true, scale: { mode: Phaser.Scale.FIT, autoCenter: Phaser.Scale.CENTER_BOTH }, input: { activePointers: 3 }, scene: new ArenaScene({ language, levelId, controls: controls.current, audio: sound, onSnapshot: setSnapshot, onFeedback: setFeedback, onReady: () => setReady(true), onIntroComplete: () => setIntro(false), onKickoffChange: setKickoff, onAwaitingNext: setAwaitingNext }) });
    }).catch(() => { if (!disposed) setLoadError(true); });
    return () => { disposed = true; game?.destroy(true); };
  }, [run, levelId, language]);
  useEffect(() => { controls.current.paused = paused || tutorial || showSettings || !ready || !entranceReady || results || recap; if (controls.current.paused) { Object.assign(controls.current, { x: 0, y: 0, boost: false, kick: false, kickRequested: false }); if (controls.current.pointer) controls.current.pointer.active = false; } }, [paused, tutorial, showSettings, ready, entranceReady, results, recap]);
  ...
  useEffect(() => { if (!feedback || (feedback.kind === 'correct' && awaitingNext)) return; const timer = setTimeout(() => setFeedback(null), feedback.kind === 'info' ? 3800 : 2300); return () => clearTimeout(timer); }, [feedback, awaitingNext]);
  useEffect(() => {
    if (!snapshot.levelComplete) return;
    saveProgress(snapshot.score, snapshot.stars, levelId, language);
    const timer = setTimeout(() => setRecap(true), 2400); return () => clearTimeout(timer);
  }, [snapshot.levelComplete, snapshot.score, snapshot.stars, levelId, language]);
  useEffect(() => {
  ...
  const replay = () => { controls.current.paused = true; controls.current.nextRoundRequested = false; setRecap(false); setResults(false); setPaused(false); setFeedback(null); setKickoff(true); setAwaitingNext(false); setSnapshot(new Match(makeLevelRounds(levelId, Math.random, language)).snapshot()); setRun(r => r + 1); };
  const nextRound = () => { controls.current.nextRoundRequested = true; setAwaitingNext(false); setFeedback(null); };
  const tutorialExample = language === 'csharp' ? ['Console.WriteLine("HI!");'] : ['print("HI!")'];
  return <main className={`game-screen screen-enter lang-${language} ${kickoff ? 'kickoff-active' : ''} ${intro ? 'arena-intro-active' : ''}`}>
    <div className="phaser-host" ref={host} role="img" aria-label="Large top-down octagonal arena with six goals and a camera following your car. Edge labels locate the ball and outputs. Steer and kick the ball into the matching output." onPointerMove={event => {
      if (event.pointerType !== 'mouse' || controls.current.paused) return;
      const canvas = host.current?.querySelector('canvas'); if (!canvas || event.target !== canvas) return;
      const bounds = canvas.getBoundingClientRect();
      controls.current.pointer = { x: (event.clientX - bounds.left) * 1200 / bounds.width, y: (event.clientY - bounds.top) * 800 / bounds.height, active: true };
    }} onPointerLeave={event => { if (event.pointerType === 'mouse' && controls.current.pointer) controls.current.pointer.active = false; }} />
    {ready && intro && !tutorial && <div className="arena-intro"><span className="eyebrow">{pack.displayName.toUpperCase()} · LEVEL {String(levelId).padStart(2, '0')}</span><h2>{level.title}.<br /><span>ONE RIGHT OUTPUT.</span></h2><p>Read the code. Find your line.</p><button className="secondary-button" onClick={() => { controls.current.skipIntro = true; }}>KICK OFF <Play size={16} /></button></div>}
    {feedback && feedback.kind !== 'info' && !recap && !results && <div key={feedback.id} className={`goal-spectacle spectacle-${feedback.kind} ${awaitingNext ? 'spectacle-awaiting' : ''}`} aria-hidden={awaitingNext ? undefined : true}><div className="goal-halo" />{feedback.kind === 'correct' && Array.from({ length: 36 }, (_, i) => <i key={i} style={{ '--i': i, '--confetti-color': ['#72e4ff', '#ffd079', '#91efb6', '#ed9dff'][i % 4] } as React.CSSProperties} />)}<div className="goal-call"><span>{feedback.kind === 'correct' ? 'THE CROWD GOES WILD' : 'TAKE A BREATH. TRY AGAIN.'}</span><b>{feedback.kind === 'correct' ? 'GOOOAL!' : 'WRONG OUTPUT'}</b>{feedback.kind === 'correct' && <code>{feedback.code?.join(' → ')} <em>→ {feedback.output}</em></code>}{awaitingNext && <button className="primary-button next-round-button" onClick={nextRound}>NEXT ROUND <ChevronRight size={23} /></button>}</div></div>}
    <header className="game-hud"><div className={`code-panel ${language === 'csharp' ? 'wide-code' : ''} ${snapshot.challenge.outputs.length > 1 ? 'mastery-code' : ''} ${snapshot.challenge.code.length > 2 ? 'expanded-code' : ''}`}><div className="code-panel-label"><span><Terminal size={14} /> {pack.displayName.toUpperCase()} · LEVEL {String(levelId).padStart(2, '0')}</span><span>{snapshot.challenge.outputs.length > 1 ? `OUTPUT ${snapshot.phase + 1} / ${snapshot.challenge.outputs.length}` : 'WHAT GETS PRINTED?'}</span></div>{snapshot.challenge.code.map((line, i) => { const outputOrder = snapshot.challenge.code.slice(0, i + 1).filter(isOutputStatement).length - 1; const done = isOutputStatement(line) && outputOrder < snapshot.phase; return <div className={`code-line ${done ? 'line-done' : ''}`} key={`${i}-${line}`}><span className="line-number">{done ? <Check size={17} /> : i + 1}</span><code><HighlightedCode line={line} /></code>{done && <Check size={18} className="success-text" />}</div>; })}</div>
