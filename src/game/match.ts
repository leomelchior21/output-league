...
export class Match {
  ...
  goal(output: string, clean: boolean): { kind: 'wrong' | 'phase' | 'correct' | 'complete'; earned: number } {
    ...
    if (output !== this.targetOutput) {
      const penalty = Math.min(20, this.score);
  ...
  nextRound() { if (this.levelComplete) return; this.round++; this.phase = 0; this.roundStartTime += this.elapsed; this.elapsed = 0; this.wrongGoalsThisRound = 0; }
  get stars() {
