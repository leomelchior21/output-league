...
export function createObstacles(kinds: ObstacleKind[], random = Math.random): Obstacle[] {
  return kinds.map((kind, i) => {
    const ring = i < 4 ? 255 : 405, angle = -Math.PI * .78 + i * Math.PI * .43 + random() * .34;
    const x = Math.cos(angle) * ring + (random() - .5) * 80, y = Math.sin(angle) * ring + (random() - .5) * 80;
    return { kind, x, y, homeX: x, homeY: y, angle: -.45 + (i % 3) * .32, width: kind === 'rotating-arm' ? 235 : 175, height: 30, radius: kind === 'slow-zone' ? 95 : 34, active: true, warning: false, vx: 0, vy: 0 };
  });
}
export function updateObstacle(o: Obstacle, time: number, dt: number) {
  ...
  if (o.kind === 'pothole') { const phase = time % 8; o.active = phase < 3.5; o.warning = phase >= 6.5; }
