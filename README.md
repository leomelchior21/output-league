...
Choose your grade, enter the supplied first-and-last-name username, then select PRINT → PLAY MATCH. Grade 7 enters Python, Grade 8 enters Swift, and Grade 9 enters C#. The staff accounts `leleomaker`, `davimaker`, and `brianmaker` exist in every grade; after signing in they can open all three languages and use the TEACH DASHBOARD button. The dashboard refreshes every five seconds while the tab is visible, separates students by school year — 7th grade in Python, 8th grade in Swift, 9th grade in C# — and lists each student’s best score, stars, and completed levels for that year’s language. Try the first-game code demo, watch the stadium reveal, then steer your rover and score the ball in the matching output goal. Settings includes a garage with four paints, three roof decals, three boost trails, and five atmosphere options. Choices sync to the active student and retain a local fallback.
