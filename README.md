...
Choose your grade, enter the supplied first-and-last-name username, then select PRINT → PLAY MATCH. Grade 7 enters Python, Grade 8 enters Swift, and Grade 9 enters C#. The staff accounts `leleomaker`, `davimaker`, and `brianmaker` exist in every grade; after signing in they can open all three languages and use the TEACH DASHBOARD button. The dashboard refreshes every five seconds while the tab is visible, separates students by school year — 7th grade in Python, 8th grade in Swift, 9th grade in C# — and shows each student’s best score and stars for Levels 1–4 of that year’s language. The ALL-TIME LEADERS board ranks the top 10 per language by combined level bests and lists every level score for each student. Try the first-game code demo, watch the stadium reveal, then steer your rover and score the ball in the matching output goal. Settings includes a garage with four paints, three roof decals, three boost trails, and five atmosphere options. Choices sync to the active student and retain a local fallback.
...
- Each language map includes eight levels. Levels 1–4 are open from the first visit; Levels 5–8 remain marked coming soon.
